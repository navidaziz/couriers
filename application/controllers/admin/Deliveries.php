...
parent::__construct();
     ...
     */
    public function view($tab = 'Pending'){
        $this->data['tab'] = $tab;
    ...
    $this->data["courier_services"] = $this->delivery_model->getList("courier_services", "courier_service_id", "courier_service_name", $where ="`courier_services`.`status` IN (1) ");
        ...
        $this->data["title"] = $this->lang->line('Add New Delivery');$this->data["view"] = ADMIN_DIR."deliveries/add_delivery";
        $this->load->view(ADMIN_DIR."layout", $this->data);
     }
     //--------------------------------------------------------------------
    ...
    //-----------------------------------------------------
     ...
     public function deliveries() {
        ...
        $search = $this->db->escape("%".$this->input->post("search")["value"]."%");
            ...
            $sql = "SELECT deliveries.*, courier_services.short_name FROM deliveries 
            INNER JOIN courier_services ON (courier_services.courier_service_id = deliveries.courier_service_id) 
            ...
            // Total records count
            $total_records = $this->db->query("SELECT COUNT(*) as count FROM deliveries WHERE delivery_status = ".$delivery_status."")->row()->count;
...
private function  add_package_to_rider($rider_id, $delivery_id){
    ...
    if($delivery){
        if(is_null($delivery->rider_id)){
            ...
            echo 'Tracking No. (<strong>'.$delivery->tracking_number.'</strong>) Package Status: '.$delivery->delivery_status;
        }
    }else{
...
public function get_rider_assigned_packages(){
    $this->data['rider_id'] = $rider_id =  (int) $this->input->post('rider_id');
    $query = "SELECT * FROM users WHERE user_id = ?";
    $this->data['rider'] = $this->db->query($query, [$rider_id])->row();
    $this->load->view(ADMIN_DIR."deliveries/rider_assigned_list", $this->data); 
}
